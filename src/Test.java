...
public class Test {
    ...
    public static String createNewUser(User user) throws IOException, InterruptedException {
        HttpResponse<String> response = CLIENT.send(Http.createUser(user),HttpResponse.BodyHandlers.ofString());
        return response.body();
    }
    public static String changeUser (User user) throws IOException, InterruptedException {
        user.setName("Aleks");
        HttpResponse<String> response = CLIENT.send(Http.changeUser("1",user), HttpResponse.BodyHandlers.ofString());
        return response.body();
    }
    public static int deleteUser(User user) throws IOException, InterruptedException {
        HttpResponse<String> re = CLIENT.send(Http.deleteUser("11", user), HttpResponse.BodyHandlers.ofString());
        return re.statusCode();
    }
    public static String getUsers() throws IOException, InterruptedException {
        HttpResponse<String> response = CLIENT.send(Http.getUsers(), HttpResponse.BodyHandlers.ofString());
        return response.body();

    }
    public static String getUserById(String id) throws IOException, InterruptedException {
        HttpResponse<String> response = CLIENT.send(Http.getUserById(id), HttpResponse.BodyHandlers.ofString());
        return response.body();
    }
    public static String getUserByUserName(String userName) throws IOException, InterruptedException {
        HttpResponse<String> response = CLIENT.send(Http.getUserByUserName(userName), HttpResponse.BodyHandlers.ofString());
        return response.body();
    }
    public static void getUserPostsAndComments(User user) throws IOException, InterruptedException {
        HttpResponse<String> response = CLIENT.send(Http.getUserPosts(user), HttpResponse.BodyHandlers.ofString());
        ...
        HttpResponse<Path> comments = CLIENT.send(Http.getComments(maxPost), HttpResponse.BodyHandlers.ofFile(Paths.get(fileName)));
    ...
    public static List<ToDos> getNotComplitedTodos(User user) throws IOException, InterruptedException {
        HttpResponse<String> response = CLIENT.send(Http.getUserToDos(user), HttpResponse.BodyHandlers.ofString());
    ...
    public static void main(String[] args) {
        User MV = new User("Aleksandr", "Bondarchuk", 1, "Pobedy ave.",
                "adm@gmail.com","38012 345 67 89","Sasha Sasha");
